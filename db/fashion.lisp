...
(defparameter +mnist-location+ "/Users/Sungjin/FMNIST")
...
(defun read-fashion-train-images (&key (path +mnist-location+) (normalize nil) (verbose nil))
  (read-fashion-images (strcat path "/train-images-idx3-ubyte")
                     :normalize normalize :verbose verbose))

(defun read-fashion-train-labels (&key (path +mnist-location+) (verbose nil) (onehot nil))
  (read-fashion-labels (strcat path "/train-labels-idx1-ubyte")
                       :onehot onehot
                       :verbose verbose))

(defun read-fashion-t10k-images (&key (path +mnist-location+) (normalize nil) (verbose nil))
  (read-fashion-images (strcat path "/t10k-images-idx3-ubyte")
                       :normalize normalize :verbose verbose))

(defun read-fashion-t10k-labels (&key (path +mnist-location+) (onehot nil) (verbose nil))
  (read-fashion-labels (strcat path "/t10k-labels-idx1-ubyte")
                       :onehot onehot
                       :verbose verbose))

(defun read-fashion-data (&key (path +mnist-location+) (normalize T) (onehot T))
