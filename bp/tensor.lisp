...
(defmethod $dlconv2d ((x tensor) (k tensor) &optional b (dw 1) (dh 1) (pw 0) (ph 0) (dlw 0) (dlh 0))
  (let ((out ($empty x))
    ...
    (nn-spatial-dilated-convolution-update-output x out k b f df ($size k 2) ($size k 3)
                                                  dw dh pw ph dlw dlh)
    out))
(defmethod $dlconv2d ((x parameter) (k parameter) &optional b (dw 1) (dh 1) (pw 0) (ph 0)
                                                    (dlw 0) (dlh 0))
...
(defmethod $dlmaxpool2d ((x tensor) kw kh &optional (dw 1) (dh 1) (pw 0) (ph 0) (dlw 0) (dlh 0)
  ...
  (let ((out ($empty x))
    ...
    out))
(defmethod $dlmaxpool2d ((x parameter) kw kh &optional (dw 1) (dh 1) (pw 0) (ph 0) (dlw 0) (dlh 0)
                                               ceilp)
  (declare (ignore kw kh dw dh pw ph dlw dlh ceilp))
  (error "not implemented yet"))

(defmethod $conv2 ((x parameter) (k parameter) &optional (type :valid))
  ($operation ($conv2 ($data x) ($data k) type)
              :creators (list x k)
              :name :conv2
              :bfn (lambda (self gradient xd kd)
                     ($bp! x ($xcorr2 gradient kd :full) self)
                     ($bp! k ($conv2 xd gradient) self))))
(defmethod $conv2 ((x tensor) (k parameter) &optional (type :valid))
  ($operation ($conv2 x ($data k) type)
              :creators (list k)
              :name :conv2
              :bfn (lambda (self gradient kd)
                     (declare (ignore kd))
                     ($bp! k ($conv2 x gradient) self))))
(defmethod $conv2 ((x parameter) (k tensor) &optional (type :valid))
  ($operation ($conv2 ($data x) k type)
              :creators (list x)
              :name :conv2
              :bfn (lambda (self gradient xd)
                     (declare (ignore xd))
                     ($bp! x ($xcorr2 gradient k :full) self))))
    ...
    ($operation out
                ...
                :bfn (lambda (self gradient &rest ignored)
                       ...
                       (let ((dx ($zero xd))
                             (dk ($zero kd))
                             (db ($zero bd)))
                         (nn-spatial-full-convolution-update-grad-input xd gradient dx kd f
                                                                        kw kh dw dh pw ph aw ah)
                         (nn-spatial-full-convolution-acc-grad-parameters xd gradient dk db f df
                                                                          kw kh dw dh pw ph aw ah
                                                                          1)
                         (when xp ($bp! x dx self))
                         (when kp ($bp! k dk self))
    ...
    ($operation out
                ...
                :bfn (lambda (self gradient &rest ignored)
                       ...
                       (let ((dx ($zero xd))
                         ...
                         (nn-spatial-full-convolution-update-grad-input xd gradient dx kd f
                                                                        kw kh dw dh pw ph aw ah)
                         (nn-spatial-full-convolution-acc-grad-parameters xd gradient dk nil f df
                                                                          kw kh dw dh pw ph aw ah
                                                                          1)
                         (when xp ($bp! x dx self))
                         (when kp ($bp! k dk self)))))))
(defmethod $dconv2d ((x parameter) (k parameter) &optional b (dw 1) (dh 1) (pw 0) (ph 0)
                                                   (aw 0) (ah 0))
  (if b
      (dconv2d-with-b x T k T b (typep b 'parameter) dw dh pw ph aw ah)
      (dconv2d-without-b x T k T dw dh pw ph aw ah)))
(defmethod $dconv2d ((x tensor) (k parameter) &optional b (dw 1) (dh 1) (pw 0) (ph 0)
                                                (aw 0) (ah 0))
  (if b
      (dconv2d-with-b x nil k T b (typep b 'parameter) dw dh pw ph aw ah)
      (dconv2d-without-b x nil k T dw dh pw ph aw ah)))
(defmethod $dconv2d ((x parameter) (k tensor) &optional b (dw 1) (dh 1) (pw 0) (ph 0)
                                                (aw 0) (ah 0))
  (if b
      (dconv2d-with-b x T k nil b (typep b 'parameter) dw dh pw ph aw ah)
