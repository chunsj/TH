...
(defgeneric $lstm (x pout pstate wi ui wf uf wo uo wa ua bi bf bo ba &optional ones)
...
(defun rnn-without-bias (x1 w1 x2 w2)
  (let ((dim1 ($ndim x1))
        (dim2 ($ndim x2)))
    (when (eq dim1 dim2)
      (let ((nf1 ($size x1 0))
...
(defun rnn-with-bias (x1 w1 x2 w2 b os)
  (let ((dim1 ($ndim x1))
        (dim2 ($ndim x2)))
    (when (eq dim1 dim2)
      (let ((nf1 ($size x1 0))
...
(defmethod $rnn ((x tensor) (ph tensor) (wx tensor) (wh tensor) (b tensor) &optional ones)
  (cond ((null b) (rnn-without-bias x wx ph wh))
        (t (rnn-with-bias x wx ph wh b ones))))

(defmethod $lstm ((x tensor) (pout tensor) (pstate tensor)
                  (wi tensor) (ui tensor)
                  (wf tensor) (uf tensor)
                  (wo tensor) (uo tensor)
                  (wa tensor) (ua tensor)
                  (bi tensor) (bf tensor) (bo tensor) (ba tensor) &optional ones)
  (let* ((it ($sigmoid ($affine2 x wi pout ui bi ones)))
         (ft ($sigmoid ($affine2 x wf pout uf bf ones)))
         (ot ($sigmoid ($affine2 x wo pout uo bo ones)))
         (at ($tanh ($affine2 x wa pout ua ba ones)))
         (state ($addm2 at it ft pstate))
         (out ($mul ($tanh state) ot)))
    (cons out state)))
...
(defmethod $rnn ((x node) (ph node) (wx node) (wh node) (b node) &optional ones)
  (node ($rnn ($data x) ($data ph) ($data wx) ($data wh) (when b ($data b)) ones)
        :name :rnn
        :link (link
...
(defmethod $lstm ((x node) (pout node) (pstate node)
                  (wi node) (ui node)
                  (wf node) (uf node)
                  (wo node) (uo node)
                  (wa node) (ua node)
                  (bi node) (bf node) (bo node) (ba node) &optional ones)
  (let* ((it ($sigmoid ($affine2 x wi pout ui bi ones)))
         (ft ($sigmoid ($affine2 x wf pout uf bf ones)))
         (ot ($sigmoid ($affine2 x wo pout uo bo ones)))
         (at ($tanh ($affine2 x wa pout ua ba ones)))
         (ct ($addm2 at it ft pstate))
         (out ($mul ($tanh ct) ot)))
    out))
...
(defmethod $lstm ((x tensor) (pout node) (pstate node)
                  (wi node) (ui node)
                  (wf node) (uf node)
                  (wo node) (uo node)
                  (wa node) (ua node)
...
(defmethod $lstm ((x node) (pout tensor) (pstate tensor)
                  (wi node) (ui node)
                  (wf node) (uf node)
                  (wo node) (uo node)
                  (wa node) (ua node)
