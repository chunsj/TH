...
(with-max-heap ()
  (let* ((train-env (cartpole-regulator-env :train))
         ...
         (total-cost ecost)
         (success nil)
         (fmt "EPOCH ~4D | TRAIN ~3D / ~4,2F | EVAL ~4D / ~5,2F | TRAIN.LOSS ~,4F"))
    (loop :repeat 500
          :while (not success)
          :for epoch :from 1
          :do (let* ((exsi (collect-experiences train-env (best-action-selector model)))
                     (exs (car exsi))
                     (ecost (cadr exsi)))
                ...
                (let* ((xys (generate-patterns model experiences 0.95D0))
                       ...
                       (esf ($1 eres))
                       (evcost ($2 eres))
                       (report (format nil fmt
                                       epoch ($count exs) ecost
                                       ne evcost loss)))
                  (setf success esf)
                  (when (or esf (zerop (rem epoch 20))) (prn report)))))))
