...
(defmethod tensor-expand ((tensor byte-tensor) (src byte-tensor) (size list))
  (when (> ($count size) 0)
    (let ((storage (long-storage ($count size)))
          (tensor (or tensor (byte-tensor))))
      (loop :for i :from 0 :below ($count size)
            :do (setf ($ storage i) ($ size i)))
      (th-byte-tensor-expand (tensor-handle tensor) (tensor-handle src)
                             (storage-handle storage))
      tensor)))
(defmethod tensor-expand ((tensor char-tensor) (src char-tensor) (size list))
  (when (> ($count size) 0)
    (let ((storage (long-storage ($count size)))
          (tensor (or tensor (char-tensor))))
      (loop :for i :from 0 :below ($count size)
            :do (setf ($ storage i) ($ size i)))
      (th-char-tensor-expand (tensor-handle tensor) (tensor-handle src)
                             (storage-handle storage))
      tensor)))
(defmethod tensor-expand ((tensor short-tensor) (src short-tensor) (size list))
  (when (> ($count size) 0)
    (let ((storage (long-storage ($count size)))
          (tensor (or tensor (short-tensor))))
      (loop :for i :from 0 :below ($count size)
            :do (setf ($ storage i) ($ size i)))
      (th-short-tensor-expand (tensor-handle tensor) (tensor-handle src)
                              (storage-handle storage))
      tensor)))
(defmethod tensor-expand ((tensor int-tensor) (src int-tensor) (size list))
  (when (> ($count size) 0)
    (let ((storage (long-storage ($count size)))
          (tensor (or tensor (int-tensor))))
      (loop :for i :from 0 :below ($count size)
            :do (setf ($ storage i) ($ size i)))
      (th-int-tensor-expand (tensor-handle tensor) (tensor-handle src)
                            (storage-handle storage))
      tensor)))
(defmethod tensor-expand ((tensor long-tensor) (src long-tensor) (size list))
  (when (> ($count size) 0)
    (let ((storage (long-storage ($count size)))
          (tensor (or tensor (long-tensor))))
      (loop :for i :from 0 :below ($count size)
            :do (setf ($ storage i) ($ size i)))
      (th-long-tensor-expand (tensor-handle tensor) (tensor-handle src)
                             (storage-handle storage))
      tensor)))
(defmethod tensor-expand ((tensor float-tensor) (src float-tensor) (size list))
  (when (> ($count size) 0)
    (let ((storage (long-storage ($count size)))
          (tensor (or tensor (float-tensor))))
      (loop :for i :from 0 :below ($count size)
            :do (setf ($ storage i) ($ size i)))
      (th-float-tensor-expand (tensor-handle tensor) (tensor-handle src)
                              (storage-handle storage))
      tensor)))
(defmethod tensor-expand ((tensor double-tensor) (src double-tensor) (size list))
  (when (> ($count size) 0)
    (let ((storage (long-storage ($count size)))
          (tensor (or tensor (double-tensor))))
      (loop :for i :from 0 :below ($count size)
            :do (setf ($ storage i) ($ size i)))
      (th-double-tensor-expand (tensor-handle tensor) (tensor-handle src)
                               (storage-handle storage))
      tensor)))
