...
(defmethod tensor-expand ((tensor tensor.double) (src tensor.double) (size list))
  (when (> ($count size) 0)
    (let ((storage (storage.long ($count size)))
          (tensor (or tensor (tensor.double))))
      (loop :for i :from 0 :below ($count size)
            :do (setf ($ storage i) ($ size i)))
...
(defmethod tensor-set-storage ((tensor tensor.byte) (storage storage.byte) offset
                               (size list) (stride list))
  (let ((size (storage.long size))
        (stride (storage.long stride)))
    (th-byte-tensor-set-storage ($handle tensor) ($handle storage) offset
                                ($handle size) ($handle stride))))
(defmethod tensor-set-storage ((tensor tensor.byte) (storage storage.byte) offset
                               (size storage.long) (stride storage.long))
  (th-byte-tensor-set-storage ($handle tensor) ($handle storage) offset
                              ($handle size) ($handle stride)))
(defmethod tensor-set-storage ((tensor tensor.char) (storage storage.char) offset
                               (size list) (stride list))
  (let ((size (storage.long size))
        (stride (storage.long stride)))
    (th-char-tensor-set-storage ($handle tensor) ($handle storage) offset
                                ($handle size) ($handle stride))))
(defmethod tensor-set-storage ((tensor tensor.char) (storage storage.char) offset
                               (size storage.long) (stride storage.long))
  (th-char-tensor-set-storage ($handle tensor) ($handle storage) offset
                              ($handle size) ($handle stride)))
(defmethod tensor-set-storage ((tensor tensor.short) (storage storage.short) offset
                               (size list) (stride list))
  (let ((size (storage.long size))
        (stride (storage.long stride)))
    (th-short-tensor-set-storage ($handle tensor) ($handle storage) offset
                                 ($handle size) ($handle stride))))
(defmethod tensor-set-storage ((tensor tensor.short) (storage storage.short) offset
                               (size storage.long) (stride storage.long))
  (th-short-tensor-set-storage ($handle tensor) ($handle storage) offset
                               ($handle size) ($handle stride)))
(defmethod tensor-set-storage ((tensor tensor.int) (storage storage.int) offset
                               (size list) (stride list))
  (let ((size (storage.long size))
        (stride (storage.long stride)))
    (th-int-tensor-set-storage ($handle tensor) ($handle storage) offset
                               ($handle size) ($handle stride))))
(defmethod tensor-set-storage ((tensor tensor.int) (storage storage.int) offset
                               (size storage.long) (stride storage.long))
  (th-int-tensor-set-storage ($handle tensor) ($handle storage) offset
                             ($handle size) ($handle stride)))
(defmethod tensor-set-storage ((tensor tensor.long) (storage storage.long) offset
                               (size list) (stride list))
  (let ((size (storage.long size))
        (stride (storage.long stride)))
    (th-long-tensor-set-storage ($handle tensor) ($handle storage) offset
                                ($handle size) ($handle stride))))
(defmethod tensor-set-storage ((tensor tensor.long) (storage storage.long) offset
                               (size storage.long) (stride storage.long))
  (th-long-tensor-set-storage ($handle tensor) ($handle storage) offset
                              ($handle size) ($handle stride)))
(defmethod tensor-set-storage ((tensor tensor.float) (storage storage.float) offset
                               (size list) (stride list))
  (let ((size (storage.long size))
        (stride (storage.long stride)))
    (th-float-tensor-set-storage ($handle tensor) ($handle storage) offset
                                 ($handle size) ($handle stride))))
(defmethod tensor-set-storage ((tensor tensor.float) (storage storage.float) offset
                               (size storage.long) (stride storage.long))
  (th-float-tensor-set-storage ($handle tensor) ($handle storage) offset
                               ($handle size) ($handle stride)))
(defmethod tensor-set-storage ((tensor tensor.double) (storage storage.double) offset
                               (size list) (stride list))
  (let ((size (storage.long size))
        (stride (storage.long stride)))
    (th-double-tensor-set-storage ($handle tensor) ($handle storage) offset
                                  ($handle size) ($handle stride))))
(defmethod tensor-set-storage ((tensor tensor.double) (storage storage.double) offset
                               (size storage.long) (stride storage.long))
  (th-double-tensor-set-storage ($handle tensor) ($handle storage) offset
...
(defmethod tensor-bitxor ((tensor tensor.int) (src tensor.int) (value number))
  (let ((tensor (or tensor (tensor.int))))
...
(defmethod tensor-zeros ((tensor tensor.byte) (src tensor.byte))
  (th-byte-tensor-zeros ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-zeros ((tensor tensor.char) (src tensor.char))
  (th-char-tensor-zeros ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-zeros ((tensor tensor.short) (src tensor.short))
  (th-short-tensor-zeros ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-zeros ((tensor tensor.int) (src tensor.int))
  (th-int-tensor-zeros ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-zeros ((tensor tensor.long) (src tensor.long))
  (th-long-tensor-zeros ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-zeros ((tensor tensor.float) (src tensor.float))
  (th-float-tensor-zeros ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-zeros ((tensor tensor.double) (src tensor.double))
  (th-double-tensor-zeros ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-ones ((tensor tensor.byte) (src tensor.byte))
  (th-byte-tensor-ones ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-ones ((tensor tensor.char) (src tensor.char))
  (th-char-tensor-ones ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-ones ((tensor tensor.short) (src tensor.short))
  (th-short-tensor-ones ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-ones ((tensor tensor.int) (src tensor.int))
  (th-int-tensor-ones ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-ones ((tensor tensor.long) (src tensor.long))
  (th-long-tensor-ones ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-ones ((tensor tensor.float) (src tensor.float))
  (th-float-tensor-ones ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-ones ((tensor tensor.double) (src tensor.double))
  (th-double-tensor-ones ($handle tensor) ($size src))
  tensor)
...
(defmethod tensor-cat ((tensor tensor.byte) dimension &rest srcs)
  (cond ((null srcs) tensor)
        ...
        (t (tensor-set tensor (reduce (lambda (r e)
                                        (tensor-cat (tensor.byte) dimension r e))
                                      srcs)))))
(defmethod tensor-cat ((tensor tensor.char) dimension &rest srcs)
  (cond ((null srcs) tensor)
        ...
        ((eq 2 ($count srcs)) (progn
                                ...
                                tensor))
        (t (tensor-set tensor (reduce (lambda (r e)
                                        (tensor-cat (tensor.char) dimension r e))
                                      srcs)))))
(defmethod tensor-cat ((tensor tensor.short) dimension &rest srcs)
  (cond ((null srcs) tensor)
        ...
        ((eq 2 ($count srcs)) (progn
                                ...
                                tensor))
        (t (tensor-set tensor (reduce (lambda (r e)
                                        (tensor-cat (tensor.short) dimension r e))
                                      srcs)))))
(defmethod tensor-cat ((tensor tensor.int) dimension &rest srcs)
  (cond ((null srcs) tensor)
        ...
        ((eq 2 ($count srcs)) (progn
                                ...
                                tensor))
        (t (tensor-set tensor (reduce (lambda (r e)
                                        (tensor-cat (tensor.int) dimension r e))
                                      srcs)))))
(defmethod tensor-cat ((tensor tensor.long) dimension &rest srcs)
  (cond ((null srcs) tensor)
        ...
        ((eq 2 ($count srcs)) (progn
                                ...
                                tensor))
        (t (tensor-set tensor (reduce (lambda (r e)
                                        (tensor-cat (tensor.long) dimension r e))
                                      srcs)))))
(defmethod tensor-cat ((tensor tensor.float) dimension &rest srcs)
  (cond ((null srcs) tensor)
        ...
        ((eq 2 ($count srcs)) (progn
                                ...
                                tensor))
        (t (tensor-set tensor (reduce (lambda (r e)
                                        (tensor-cat (tensor.float) dimension r e))
                                      srcs)))))
(defmethod tensor-cat ((tensor tensor.double) dimension &rest srcs)
  (cond ((null srcs) tensor)
        ...
        ((eq 2 ($count srcs)) (progn
                                ...
                                tensor))
        (t (tensor-set tensor (reduce (lambda (r e)
                                        (tensor-cat (tensor.double) dimension r e))
                                      srcs)))))

(defmethod tensor-cat2 ((tensor tensor.byte) dimension (src1 tensor.byte) (src2 tensor.byte))
  (th-byte-tensor-cat ($handle tensor) ($handle src1) ($handle src2) dimension)
  tensor)
(defmethod tensor-cat2 ((tensor tensor.char) dimension (src1 tensor.char) (src2 tensor.char))
  (th-char-tensor-cat ($handle tensor) ($handle src1) ($handle src2) dimension)
  tensor)
(defmethod tensor-cat2 ((tensor tensor.short) dimension (src1 tensor.short) (src2 tensor.short))
  (th-short-tensor-cat ($handle tensor) ($handle src1) ($handle src2) dimension)
  tensor)
(defmethod tensor-cat2 ((tensor tensor.int) dimension (src1 tensor.int) (src2 tensor.int))
  (th-int-tensor-cat ($handle tensor) ($handle src1) ($handle src2) dimension)
  tensor)
(defmethod tensor-cat2 ((tensor tensor.long) dimension (src1 tensor.long) (src2 tensor.long))
  (th-long-tensor-cat ($handle tensor) ($handle src1) ($handle src2) dimension)
  tensor)
(defmethod tensor-cat2 ((tensor tensor.float) dimension (src1 tensor.float) (src2 tensor.float))
  (th-float-tensor-cat ($handle tensor) ($handle src1) ($handle src2) dimension)
  tensor)
(defmethod tensor-cat2 ((tensor tensor.double) dimension (src1 tensor.double) (src2 tensor.double))
  (th-double-tensor-cat ($handle tensor) ($handle src1) ($handle src2) dimension)
  tensor)
