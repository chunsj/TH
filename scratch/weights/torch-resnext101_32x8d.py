...
wlen = len(weights)
fcidx = wlen - 2 # fc with bias

dbpath = "/Users/Sungjin/Desktop/resnext101_32x8d"
...
for z in range(wlen):
  ...
  if z < fcidx:
    if dim == 4:
      fname = dbpath + "/resnext101_32x8d-p" + str(z) + ".txt"
      f = open(fname, "w")
      f.write("4\n")
    ...
    if dim == 2:
      w = w.transpose()
      fname = dbpath + "/resnext101_32x8d-f" + str(z) + ".txt"
      f = open(fname, "w")
      f.write("2\n")
      f.write(str(w.shape[0]) + "\n")
      f.write(str(w.shape[1]) + "\n")
...
for z in range(len(children)):
  ...
  if c.__class__.__name__ == 'BatchNorm2d':
    rm = c.running_mean
    rv = c.running_var
    fname = dbpath + "/resnext101_32x8d-m" + str(pidx) + ".txt"
    f = open(fname, "w")
    f.write("1\n")
    ...
    for i in range(rm.shape[0]):
      f.write(("%e" % rm[i]) + " ")
    f.close()
    fname = dbpath + "/resnext101_32x8d-v" + str(pidx) + ".txt"
    f = open(fname, "w")
    f.write("1\n")
