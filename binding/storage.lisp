...
#+ccl (defmethod ccl:terminate ((x storage.byte))
        (when (has-valid-handle-p x)
          (th-byte-storage-free ($handle x))
          (reset-handle x)))
#+ccl (defmethod ccl:terminate ((x storage.char))
        (when (has-valid-handle-p x)
          (th-char-storage-free ($handle x))
          (reset-handle x)))
#+ccl (defmethod ccl:terminate ((x storage.short))
        (when (has-valid-handle-p x)
          (th-short-storage-free ($handle x))
          (reset-handle x)))
#+ccl (defmethod ccl:terminate ((x storage.int))
        (when (has-valid-handle-p x)
          (th-int-storage-free ($handle x))
          (reset-handle x)))
#+ccl (defmethod ccl:terminate ((x storage.long))
        (when (has-valid-handle-p x)
          (th-long-storage-free ($handle x))
          (reset-handle x)))
#+ccl (defmethod ccl:terminate ((x storage.float))
        (when (has-valid-handle-p x)
          (th-float-storage-free ($handle x))
          (reset-handle x)))
#+ccl (defmethod ccl:terminate ((x storage.double))
        (when (has-valid-handle-p x)
