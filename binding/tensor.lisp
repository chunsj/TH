...
(defmethod allocate-tensor-handle ((tensor tensor.byte) &optional dimensions)
  (let ((handle (if dimensions
                    (th-byte-tensor-new-with-size ($handle (storage.long dimensions)) +nil+)
                    (th-byte-tensor-new))))
    (setf ($handle tensor) handle)
    handle))
(defmethod allocate-tensor-handle ((tensor tensor.char) &optional dimensions)
  (let ((handle (if dimensions
                    (th-char-tensor-new-with-size ($handle (storage.long dimensions)) +nil+)
                    (th-char-tensor-new))))
    (setf ($handle tensor) handle)
    handle))
(defmethod allocate-tensor-handle ((tensor tensor.short) &optional dimensions)
  (let ((handle (if dimensions
                    (th-short-tensor-new-with-size ($handle (storage.long dimensions)) +nil+)
                    (th-short-tensor-new))))
    (setf ($handle tensor) handle)
    handle))
(defmethod allocate-tensor-handle ((tensor tensor.int) &optional dimensions)
  (let ((handle (if dimensions
                    (th-int-tensor-new-with-size ($handle (storage.long dimensions)) +nil+)
                    (th-int-tensor-new))))
    (setf ($handle tensor) handle)
    handle))
(defmethod allocate-tensor-handle ((tensor tensor.long) &optional dimensions)
  (let ((handle (if dimensions
                    (th-long-tensor-new-with-size ($handle (storage.long dimensions)) +nil+)
                    (th-long-tensor-new))))
    (setf ($handle tensor) handle)
    handle))
(defmethod allocate-tensor-handle ((tensor tensor.float) &optional dimensions)
  (let ((handle (if dimensions
                    (th-float-tensor-new-with-size ($handle (storage.long dimensions)) +nil+)
                    (th-float-tensor-new))))
    (setf ($handle tensor) handle)
    handle))
(defmethod allocate-tensor-handle ((tensor tensor.double) &optional dimensions)
  (let ((handle (if dimensions
...
(defmethod deallocate-tensor-handle ((tensor tensor.byte))
  (let ((h ($handle tensor)))
    (when h
      (th-byte-tensor-free h)
      (setf ($handle tensor) nil))
    tensor))
(defmethod deallocate-tensor-handle ((tensor tensor.char))
  (let ((h ($handle tensor)))
    (when h
      (th-char-tensor-free h)
      (setf ($handle tensor) nil))
    tensor))
(defmethod deallocate-tensor-handle ((tensor tensor.short))
  (let ((h ($handle tensor)))
    (when h
      (th-short-tensor-free h)
      (setf ($handle tensor) nil))
    tensor))
(defmethod deallocate-tensor-handle ((tensor tensor.int))
  (let ((h ($handle tensor)))
    (when h
      (th-int-tensor-free h)
      (setf ($handle tensor) nil))
    tensor))
(defmethod deallocate-tensor-handle ((tensor tensor.long))
  (let ((h ($handle tensor)))
    (when h
      (th-long-tensor-free h)
      (setf ($handle tensor) nil))
    tensor))
(defmethod deallocate-tensor-handle ((tensor tensor.float))
  (let ((h ($handle tensor)))
    (when h
      (th-float-tensor-free h)
      (setf ($handle tensor) nil))
    tensor))
(defmethod deallocate-tensor-handle ((tensor tensor.double))
  (let ((h ($handle tensor)))
    (when h
      (th-double-tensor-free h)
      (setf ($handle tensor) nil))
    tensor))

(defmethod allocate-tensor ((tensor tensor.byte) &optional dimensions)
  (let ((handle (allocate-tensor-handle tensor dimensions)))
