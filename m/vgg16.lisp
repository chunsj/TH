...
(defun vgg16 (x &optional (flat :all))
  (when (and x (>= ($ndim x) 3) (equal (last ($size x) 3) (list 3 224 224)))
    (let ((weights (read-vgg16-weights (not (eq flat :none))))
          (x (if (eq ($ndim x) 3)
                 ($reshape x 1 3 224 224)
                 x)))
      (-> x
          ($conv2d (getf weights :k1) (getf weights :b1))
          ($relu)
          ($conv2d (getf weights :k2) (getf weights :b2))
          ($relu)
          ($maxpool2d 2 2 2 2)
          ($conv2d (getf weights :k3) (getf weights :b3))
          ($relu)
          ($conv2d (getf weights :k4) (getf weights :b4))
          ($relu)
          ($maxpool2d 2 2 2 2)
          ($conv2d (getf weights :k5) (getf weights :b5))
          ($relu)
          ($conv2d (getf weights :k6) (getf weights :b6))
          ($relu)
          ($conv2d (getf weights :k7) (getf weights :b7))
          ($relu)
          ($maxpool2d 2 2 2 2)
          ($conv2d (getf weights :k8) (getf weights :b8))
          ($relu)
          ($conv2d (getf weights :k9) (getf weights :b9))
          ($relu)
          ($conv2d (getf weights :k10) (getf weights :b10))
          ($relu)
          ($maxpool2d 2 2 2 2)
          ($conv2d (getf weights :k11) (getf weights :b11))
          ($relu)
          ($conv2d (getf weights :k12) (getf weights :b12))
          ($relu)
          ($conv2d (getf weights :k13) (getf weights :b13))
          ($relu)
          ($maxpool2d 2 2 2 2)
