...
(defun sum-backprop (node gradient dimension)
  ...
  (setf ($children node) (when ($children node)
                           (let ((x ($c0 node)))
                             (list (if ($gradientp x)
                                       (let* ((rfn (repeat-to-match-shape ($data x) dimension))
                                              (rr (funcall rfn gradient)))
                                         ($bp! x (car rr)))
                                       x)))))
  node)

(defun sum-backprop-simple (node gradient dimension)
...
(defun min-backprop (node gradient dimension)
  ...
  (setf ($children node) (when ($children node)
                           (let ((x ($c0 node)))
                             (list (if ($gradientp x)
                                       (let* ((rfn (repeat-to-match-shape ($data x) dimension))
                                              (rrg (funcall rfn gradient))
                                              (rrn (funcall rfn ($data node))))
                                         ($bp! x (set!-neq ($data x) (car rrn) (car rrg))))
                                       x)))))
  node)
...
(defun relu-backprop (node gradient)
  ...
  (setf ($children node) (when ($children node)
                           (let ((x ($c0 node)))
                             (list (if ($gradientp x)
                                       ($bp! x ($mul! (zeros ($data node)) gradient))
                                       x)))))
  node)
...
(defmethod $softmax ((x tensor))
  (if (or (eq ($ndim x) 1) (eq ($size x 0) 1))
      (let* ((c ($max x))
             (exp-a ($exp ($sub x c)))
             (sum-exp-a ($sum exp-a)))
        ($div exp-a sum-exp-a))
      (let ((result ($clone x)))
        (loop :for i :from 0 :below ($size result 0)
              :for row = ($ result i)
              :do (let ((c ($max row)))
                    ($sub! row c)
                    ($exp! row)
...
(defun dsoftmax (s)
  (if (or (eq ($ndim s) 1) (eq ($size s 0) 1))
      ($sub! ($diag s) ($vv s s))
      (let ((result ($empty s)))
        ($resize! result (list ($size s 0) ($size s 1) ($size s 1)))
        (loop :for i :from 0 :below ($size result 0)
              :for row = ($ s i)
              :for rr = ($ result i)
              :do ($copy! rr ($sub! ($diag row) ($vv row row))))
        result)))
