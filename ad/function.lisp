...
(defmethod $sum ((x tape) &optional dimension)
  (let ((result (tape ($sum ($data x) dimension))))
    (setf ($children result) (list x))
    (setf ($gradientp result) ($gradientp x))
...
(defmethod $mean ((x tape) &optional dimension)
  (let ((result (tape ($mean ($data x) dimension))))
    (setf ($children result) (list x))
    (setf ($gradientp result) ($gradientp x))
...
(defmethod $min ((x tape) &optional dimension)
  (let ((result (tape ($min ($data x) dimension))))
    (setf ($children result) (list x))
    (setf ($gradientp result) ($gradientp x))
...
(defmethod $max ((x tape) &optional dimension)
  (let ((result (tape ($max ($data x) dimension))))
    (setf ($children result) (list x))
    (setf ($gradientp result) ($gradientp x))
...
(defmethod $transpose ((x tape) &optional dimension0 dimension1)
  (let ((result (tape ($transpose ($data x) dimension0 dimension1))))
    (setf ($children result) (list x))
    (setf ($gradientp result) ($gradientp x))
...
(defmethod $bmm ((bx tape) (by tape))
  (let ((result (tape ($bmm ($data bx) ($data by)))))
...
(defmethod $conv2 ((x tape) (k tape) &optional (type :valid))
  (let ((result (tape ($conv2 ($data x) ($data k) type))))
...
(defmethod $xcorr2 ((x tape) (k tape) &optional (type :valid))
  (let ((result (tape ($xcorr2 ($data x) ($data k) type))))
    (setf ($children result) (list x k))
    (setf ($gradientp result) (or ($gradientp x) ($gradientp k)))
