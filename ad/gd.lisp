          ...
          (let ((data ($data node))
            ...
            (cond ((null grv) nil)
                  ((numberp grv) (let ((h ($ ($attrs node) :h 0))
                                   ...
                                   (setf h (+ h (* (- 1 decay-rate) grv grv)))
                                   (setf d (* d decay-rate))
                                   (setf d (+ d (* (- 1 decay-rate)
                                                   (expt (/ (- grv) (+ (sqrt h) 1E-8)) 2))))
                                   (setf ($ ($attrs node) :h) h)
                                   (setf ($ ($attrs node) :d) d)
                                   (setf ($data node) (- data (/ (sqrt d)
                                                                 (+ (sqrt h) 1E-8))))))
                  (t (let ((h ($ ($attrs node) :h (apply #'zeros ($size grv))))
                           (d ($ ($attrs node) :d (apply #'zeros ($size grv)))))
                       ($mul! h decay-rate)
                       ($axpy! (- 1 decay-rate) ($expt grv 2) h)
                       ($mul! d decay-rate)
                       ($axpy! (- decay-rate 1) ($expt! ($div grv ($add! ($sqrt h) 1E-8)) 2) d)
                       (prn "D" d)
                       (prn "H" h)
                       ($axpy! -1 ($mul! ($div d h) grv) data))))
            (setf ($gradient node) nil))))
    node))
