...
(defmethod $dot ((a node) (b tensor))
  (let ((result (node ($dot ($data a) b))))
    (setf ($name result) "DOT")
    ($gp! result a)
    ($pfn! a (lambda () ($mul b ($gradient result))))
    result))
...
(defmethod $mv ((m node) (v tensor))
  (let ((result (node ($mv ($data m) v))))
    (setf ($name result) "MV")
    ($gp! result m)
    ($pfn! m (lambda () ($vv ($gradient result) v)))
    result))

(defmethod $mv ((m tensor) (v node))
  (let ((result (node ($mv m ($data v)))))
    (setf ($name result) "MV")
    ($gp! result v)
    ($pfn! v (lambda () ($@ ($transpose m) ($gradient result))))
    result))
...
(defmethod $mm ((a node) (b tensor))
  (let ((result (node ($mm ($data a) b))))
    (setf ($name result) "MM")
    ($gp! result a)
    ($pfn! a (lambda () ($mm ($gradient result) ($transpose b))))
    result))

(defmethod $mm ((a tensor) (b node))
  (let ((result (node ($mm a ($data b)))))
    (setf ($name result) "MM")
    ($gp! result b)
    ($pfn! b (lambda () ($mm ($transpose a) ($gradient result))))
    result))
...
(defmethod $mul ((a node) (b tensor))
  (let ((result (node ($mul ($data a) b))))
    (setf ($name result) "MUL")
    ($gp! result a)
    ($pfn! a (lambda () ($mul b ($gradient result))))
    result))

(defmethod $mul ((a tensor) (b node))
  (let ((result (node ($mul a ($data b)))))
    (setf ($name result) "MUL")
    ($gp! result b)
    ($pfn! b (lambda () ($mul a ($gradient result))))
    result))

(defmethod $mul ((a node) (b number))
  (let ((result (node ($mul ($data a) b))))
    (setf ($name result) "MUL")
    ($gp! result a)
    ($pfn! a (lambda () ($mul b ($gradient result))))
...
(defmethod $mul ((a number) (b node)) ($mul b a))
...
(defmethod $bmm ((bx node) (by tensor))
  (let ((result (node ($bmm ($data bx) by))))
    (setf ($name result) "BMM")
    ($gp! result bx)
    ($pfn! bx (lambda () ($bmm ($gradient result) ($transpose ($data by) 2 1))))
    result))
...
(defmethod $bmm ((bx tensor) (by node))
  (let ((result (node ($bmm bx ($data by)))))
    (setf ($name result) "BMM")
    ($gp! result by)
    ($pfn! by (lambda () ($bmm ($transpose ($data bx) 2 1) ($gradient result))))
    result))
...
(defmethod $div ((a node) (b tensor))
  (let ((result (node ($div ($data a) b))))
    (setf ($name result) "DIV")
    ($gp! result a)
    ($pfn! a (lambda () ($div ($gradient result) b)))
    result))

(defmethod $div ((a tensor) (b node))
  (let ((result (node ($div a ($data b)))))
    (setf ($name result) "DIV")
    ($gp! result b)
    ($pfn! b (lambda () ($neg! ($div ($mul a ($gradient result))
                                     ($expt ($data b) 2)))))
    result))
...
(defmethod $vv ((a node) (b tensor))
  (let ((result (node ($vv ($data a) b))))
    (setf ($name result) "VV")
    ($gp! result a)
    ($pfn! a (lambda () ($mv ($gradient result) b)))
    result))

(defmethod $vv ((a tensor) (b node))
  (let ((result (node ($vv a ($data b)))))
    (setf ($name result) "VV")
    ($gp! result b)
    ($pfn! b (lambda () ($mv ($transpose ($gradient result)) a)))
    result))
...
(defmethod $cat ((x node) (y tensor) &optional (dimension 0))
  (let ((result (node ($cat ($data x) y dimension))))
    (setf ($name result) "CAT")
    ($gp! result x)
    ($pfn! x (lambda () ($narrow ($gradient result) dimension 0 ($size ($data x) 1))))
    result))
...
(defmethod $cat ((x tensor) (y node) &optional (dimension 0))
  (let ((result (node ($cat x ($data y) dimension))))
    (setf ($name result) "CAT")
    ($gp! result y)
    ($pfn! y (lambda () ($narrow ($gradient result) dimension ($size x 1)
                                 ($size ($data y) 1))))
    result))
