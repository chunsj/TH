...
(defgeneric $bn (x gamma beta &optional rm rv sm sd momentum eps))
...
(defmethod $bn ((x tensor) (gamma tensor) (beta tensor)
                &optional rm rv sm sd (momentum 0.1) (eps 1E-5))
  (let ((output ($empty x))
        (n ($size x 1)))
    (if (and rm rv sm sd)
...
(defmethod $bn ((x node) (gamma node) (beta node)
                &optional rm rv sm sd (momentum 0.1) (eps 1E-5))
